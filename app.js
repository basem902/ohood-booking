...
async function init() {
  ...
  // تاريخ اليوم والمدة الافتراضية كقيم ابتدائية
  $('#f-date').value = todayStr();
  $('#f-duration').value = settings.defaultDuration;
...
function applyWorkSettingsToUI() {
  ...
  const dd = $('#set-duration'); if (dd) dd.value = settings.defaultDuration || 60;
...
async function loadPackages() {
  ...
  if (!packages.length) {
    const seed = [
      { id: uid(), name: 'استشارة', price: 100 },
      { id: uid(), name: 'جلسة عادية', price: 200 },
      { id: uid(), name: 'باقة مميزة', price: 350 },
      { id: uid(), name: 'باقة VIP', price: 600 },
    ];
...
async function onSave(e) {
  ...
  const duration = Math.max(1, Math.round(Number($('#f-duration').value) || settings.defaultDuration));
...
function editBooking(id) {
  ...
  $('#f-duration').value = (b.duration != null ? b.duration : settings.defaultDuration);
...
function resetForm() {
  ...
  $('#f-id').value = '';
  $('#f-date').value = todayStr();
  $('#f-duration').value = settings.defaultDuration;   // المدة الافتراضية
  $('#f-paid').value = '';
...
function refreshDayWarning() {
  ...
  // التعارض الفعلي = تقاطع المجالات الزمنية مع المدة الحالية
  const duration = Math.max(1, Number($('#f-duration').value) || settings.defaultDuration);
...
async function saveWorkSettings() {
  ...
  settings.defaultDuration = Number($('#set-duration').value) || 60;
  ...
  // حدّث القيمة الافتراضية للمدة في النموذج إن لم يكن قيد تعديل
  if (!editingId) {
    const fd = $('#f-duration');
    if (fd && !fd.value) fd.value = settings.defaultDuration;
  }
  refreshAll();
  refreshDayWarning();
...
function exportCSV() {
  const headers = ['الاسم', 'الجوال', 'التاريخ', 'اليوم', 'الوقت', 'الباقة', 'السعر', 'المدفوع', 'المتبقّي', 'المدة (دقائق)', 'حالة الدفع', 'الحالة', 'ملاحظات'];
  const lines = bookings
    .slice()
    .sort((a, b) => ((a.date || '') + (a.time || '')).localeCompare((b.date || '') + (b.time || '')))
    .map((b) => [b.name, b.phone, b.date, weekdayName(b.date), b.time, b.package, bookingPrice(b), bookingPaid(b), bookingRemaining(b), bookingDuration(b), PAY_LABEL[paymentStatus(b)] || '', STATUS_LABEL[b.status] || '', b.notes]
